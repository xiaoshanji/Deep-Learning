...
# 增加 .astype(float) 兼容新版 Pandas 防止 bool 类型引发 Tensor 转换错误
...
def k_fold(k, X_train, y_train, num_epochs, learning_rate, weight_decay, batch_size):
    ...
    for i in range(k):
        ...
        if i == 0:
            ...
            plt.plot(range(1, num_epochs + 1), valid_ls, label='valid', linestyle='--')
            plt.xlabel('epoch')
            plt.ylabel('rmse')
            plt.yscale('log')
            plt.legend()
